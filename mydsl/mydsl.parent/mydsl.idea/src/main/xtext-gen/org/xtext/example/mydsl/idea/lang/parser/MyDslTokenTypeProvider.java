...
@Singleton public class MyDslTokenTypeProvider implements TokenTypeProvider {
	...
	private static final String[] TOKEN_NAMES = new PsiInternalMyDslParser(null).getTokenNames();

	private static final IElementType[] tokenTypes = new IElementType[TOKEN_NAMES.length];
	
	static {
		for (int i = 0; i < TOKEN_NAMES.length; i++) {
			tokenTypes[i] = new IndexedElementType(TOKEN_NAMES[i], i, MyDslLanguage.INSTANCE);
		}
	}

	private static final TokenSet WHITESPACE_TOKENS = TokenSet.create(tokenTypes[PsiInternalMyDslParser.RULE_WS]);
	private static final TokenSet COMMENT_TOKENS = TokenSet.create(tokenTypes[PsiInternalMyDslParser.RULE_SL_COMMENT], tokenTypes[PsiInternalMyDslParser.RULE_ML_COMMENT]);
	private static final TokenSet STRING_TOKENS = TokenSet.create(tokenTypes[PsiInternalMyDslParser.RULE_STRING]);
